...
class inactive_bug_tracker(object):
    def __init__(self):
        ...
        self.reset_message = ("I have reset the assignee as a part of an automated"
                              "cleanup since there has been no visible activity in"
                              "this bug for some time. If this is incorrect, please"
                              "let me know and I'll correct the error")
...
if __name__ == '__main__':
    # allow the user to do a dry run. Instead of making changes the program
    #  will just find the bugs to change and print the changes that it would make
    script, run_type = argv
    IMBT = inactive_bug_tracker()
    if run_type == 'live':
        IMBT.main()
    elif run_type == 'dry_run':
        IMBT.dry()
